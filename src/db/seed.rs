use crate::db::{lockers, payments, rentals};
...
pub fn seed_test_data(conn: &Connection) -> Result<()> {
    ...
    conn.execute(
        "UPDATE lockers SET is_damaged = 1 WHERE label IN ('A-015', 'B-025', 'C-010')",
        [],
    )?;
    ...
    for (label, username, tenant_type, start_offset, end_offset, deposit_paid, deposit_returned) in
        returned_rentals
    {
        let locker = lockers::get_locker_by_label(conn, label)?.unwrap();
        let mut rental = Rental::new(
